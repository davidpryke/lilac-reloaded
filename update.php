...
require_once('includes/lilac-conf.php');
...
if($stage == 1) {
	...
	?>
<div class="notice">
	<strong>Note:</strong> Lilac supports Nagios 3.x only. It is possible
	to import Nagios 2.x configuration files, but Lilac will only export to
	Nagios 3.x.
</div>
<div class="checks">
	<?php
	...
	<div class="<?php if($fail) echo "failure"; else echo "success";?>">
		Configuration File Writable</div>
	<?php
	if($fail) {
		?>
	<div class="error">
		<p>
			The Lilac installer requires that it write to the configuration file
			at <em><?php echo dirname(__FILE__) . "/includes/lilac-conf.php";?> </em>.
			It is recommended that you change the permissions of the includes
			directory so the web user can write to it. Following steps are
			possible:
		</p>
		<p>
			- Temporary set the file permission of directory <em><?php echo dirname(__FILE__) . "/includes";?>
			</em> to 777. After installation set back to defaults.<br /> -
			Temporary set the owner permission of directory <em><?php echo dirname(__FILE__) . "/includes";?>
			</em> to the webserveruser. After installation set back to defaults.<br />
			- Create the file <em><?php echo dirname(__FILE__) . "/includes/lilac-conf.php";?>
			</em> by yourself and change owner to webserveruser.<br />
		</p>
	</div>
	<?php
	}
	...
	?>
	<div class="<?php if($fail) echo "failure"; else echo "success";?>">
		PHP Pear Library Installed</div>
	<?php
	if($fail) {
		?>
	<div class="error">
		PHP's PEAR library must be loaded. Please refer to <a
			href="http://pear.php.net">PHP's PEAR Homepage</a> for more
		information.
	</div>
	<?php
	}
	...
	?>
	<div class="<?php if($fail) echo "failure"; else echo "success";?>">
		PHP PDO MySQL Extension Loaded</div>
	<?php
	if($fail) {
		?>
	<div class="error">
		PHP's PDO MySQL extension must be loaded. This is not the same as the
		MySQLi extension. Please refer to <a
			href="http://us3.php.net/manual/en/ref.pdo.html">PHP's PDO
	...
	?>
	<div class="<?php if($fail) echo "failure"; else echo "success";?>">
		MySQL Client Executable</div>
	<?php
	if($fail) {
		?>
	<div class="notice">The Lilac installer needs the MySQL client binary
		to be installed and executable by the webserver if you need to
		populate the Lilac database. Install the MySQL command line utility.</div>
	<?php
	}
	...
	if($fail) {
		?>
	<div class="notice">Not a fatal error, Lilac needs the NMAP binary to
		be installed and executable by the webserver to perform auto-discovery
		tasks. Install the NMAP command line utility. If you are not going to
		perform auto-discovery tasks, you can ignore this and continue. If you
		choose to do auto-discovery tasks in the future, you must have NMAP
		installed.</div>
	<?php
	}
	?>
	...
	?>
	<div class="<?php if($fail) echo "failure"; else echo "success";?>">
		PHP Curl Support</div>
	<?php
	if($fail) {
		?>
	<div class="error">
		PHP's Curl support is not available. Curl is required for for Lilac to
		call internal webservices.. For more information, refer to <a
			href="http://www.php.net/manual/en/curl.setup.php">PHP'S Curl
			Extension setup guide</a> for more information on how to install it.
	</div>
	<?php
	}
	if($fail) $fatalErrors = true;
	...
	?>
	<div class="<?php if($fail) echo "failure"; else echo "success";?>">
		PHP SimpleXML Support</div>
	<?php
	if($fail) {
		?>
	<div class="error">
		PHP's SimpleXML support is not available. SimpleXML is required for
		parsing XML documents and is used by Lilac's Autodiscovery system. For
		more information, refer to <a
			href="http://www.php.net/manual/en/simplexml.setup.php">PHP'S
			SimpleXML Extension setup guide</a> for more information on how to
		install it.
	</div>
	<?php
	}
	...
	?>
	<div class="<?php if($fail) echo "failure"; else echo "success";?>">
		PHP POSIX Support</div>
	<?php
	if($fail) {
		?>
	<div class="error">
		PHP's POSIX support is not available. POSIX support is required for
		the importer/exporter/autodiscovery to function. For instructions,
		refer to <a href="http://us3.php.net/manual/en/posix.setup.php">PHP's
			POSIX Extension setup guide</a> for more information on how to
		install it.
	</div>
	<?php
	}
	...
	?>
	<div class="<?php if($fail) echo "failure"; else echo "success";?>">
		PHP JSON Support</div>
	<?php
	if($fail) {
		?>
	<div class="error">
		PHP's JSON support is not available. JSON support is automatically
		provided in PHP 5.2.0. For previous versions of PHP, refer to <a
			href="http://us3.php.net/manual/en/json.setup.php">PHP's JSON
			Extension setup guide</a> for more information on how to install it.
	</div>
	<?php
	}
	...
	if($fail) {
		?>
	<div class="error">
		PHP's XML support is not available. See <a
			href="http://www.php.net/manual/en/book.xml.php">XML Parser</a> how
		to enable it.
	</div>
	<?php
	}
	...
	?>
	<div class="<?php if($fail) echo "failure"; else echo "success";?>">
		PHP Command Line Interface (CLI) Available</div>
	<?php
	if($fail) {
		?>
	<div class="error">
		Lilac must be able to run command line PHP script to import and export
		configurations. Please make sure PHP's command line interface is
		installed and the PHP binary is in the search path for the web user.
		Refer to <a href="http://www.php.net">The PHP Group's Homepage</a> for
		more information.
	</div>
	<?php
	}
	...
	<div class="checks">
		...
		?>
		<div class="<?php if($fail) echo "failure"; else echo "success";?>">
			CLI JSON Support</div>
		<?php
		if($fail) {
			?>
		<div class="error">
			JSON Support must be provided to PHP's command line interface. This
			is automatically provided in PHP 5.2.0 and above. For previous
			versions of PHP, refer to <a
				href="http://us3.php.net/manual/en/json.setup.php">PHP's JSON
				Extension setup guide</a> for more information on how to install it.
		</div>
		<?php
		...
		?>
		<div class="<?php if($fail) echo "warning"; else echo "success";?>">
			CLI Memory Limit For Scripts:
			<?php echo $clisupport['memorysize']; ?>
		</div>
		<?php
		if($fail) {
			?>
		<div>
			Not Really an Error, but a note, PHP's Memory Limit for the command
			line scripts must be set to either -1 (Unlimited) or to a reasonable
			amount according to the size of your configuration. Suggested amount
			is 64M. Refer to <a
				href="http://www.php.net/manual/en/ini.core.php#ini.memory-limit">php.ini
				directives</a> for more information on how to change this value.
		</div>
		<?php
		}
		?>
		...
		?>
		<div class="<?php if($fail) echo "failure"; else echo "success";?>">
			CLI Process Control Support</div>
		<?php
		if($fail) {
			?>
		<div class="error">
			PHP must have Process control compiled in in order to properly
			import/export and auto-discover. For more information on how to
			install process control for PHP, refer to <a
				href="http://www.php.net/manual/en/pcntl.setup.php">PHP's PCNTL
		...
		?>
		<div class="<?php if($fail) echo "failure"; else echo "success";?>">
			CLI PDO MySQL Support</div>
		<?php
		if($fail) {
			?>
		<div class="error">
			MySQL Support must be provided to PHP's command line interface. This
			is not the same as the MySQLi extension. Please refer to <a
				href="http://us3.php.net/manual/en/ref.pdo.html">PHP's PDO
				documentation</a> for more information.
		</div>
		<?php
		...
		if($fail) {
			?>
		<div class="error">
			XML Support must be provided to PHP's command line interface. See <a
				href="http://www.php.net/manual/en/book.xml.php">XML Parser</a> how
			to enable it.
		</div>
		<?php
		...
		?>
		<div class="<?php if($fail) echo "failure"; else echo "success";?>">
			CLI SimpleXML Support</div>
		<?php
		if($fail) {
			?>
		<div class="error">
			PHP's SimpleXML support is not available for PHP's command line
			interface. SimpleXML is required for parsing XML documents and is
			used by Lilac's Autodiscovery system. For more information, refer to
			<a href="http://www.php.net/manual/en/simplexml.setup.php">PHP'S
				SimpleXML Extension setup guide</a> for more information on how to
			install it.
		</div>
		<?php
...
<div class="checks">
	...
	<div class="error">
		<p>
			The lilac-reloaded updater requires that the configuration file
			at <em><?php echo dirname(__FILE__) . "/includes/lilac-conf.php";?> </em> is present.<br>
			Please check if your installation is in a sane situation.
		</p>
	</div>
</div>
<?php 
