...
abstract class BaseNagiosServiceTemplateInheritanceQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = NagiosServiceTemplateInheritancePeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
	 ...
	 *
	 * @return    NagiosServiceTemplateInheritance A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con)
	{
		$sql = 'SELECT `ID`, `SOURCE_SERVICE`, `SOURCE_TEMPLATE`, `TARGET_TEMPLATE`, `ORDER` FROM `nagios_service_template_inheritance` WHERE `ID` = :p0';
		try {
