...
abstract class BaseNagiosContactAddressQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = NagiosContactAddressPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
	 ...
	 *
	 * @return    NagiosContactAddress A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con)
	{
		$sql = 'SELECT `ID`, `CONTACT`, `ADDRESS` FROM `nagios_contact_address` WHERE `ID` = :p0';
		try {
