...
abstract class BaseNagiosEscalationContactgroupQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = NagiosEscalationContactgroupPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
	 ...
	 *
	 * @return    NagiosEscalationContactgroup A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con)
	{
		$sql = 'SELECT `ID`, `ESCALATION`, `CONTACTGROUP` FROM `nagios_escalation_contactgroup` WHERE `ID` = :p0';
		try {
