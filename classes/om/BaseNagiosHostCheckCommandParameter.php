...
abstract class BaseNagiosHostCheckCommandParameter extends BaseObject  implements Persistent
{
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = NagiosHostCheckCommandParameterQuery::create()
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(NagiosHostCheckCommandParameterPeer::ID)) {
			$modifiedColumns[':p' . $index++]  = '`ID`';
		}
		if ($this->isColumnModified(NagiosHostCheckCommandParameterPeer::HOST)) {
			$modifiedColumns[':p' . $index++]  = '`HOST`';
		}
		if ($this->isColumnModified(NagiosHostCheckCommandParameterPeer::HOST_TEMPLATE)) {
			$modifiedColumns[':p' . $index++]  = '`HOST_TEMPLATE`';
		}
		if ($this->isColumnModified(NagiosHostCheckCommandParameterPeer::PARAMETER)) {
			$modifiedColumns[':p' . $index++]  = '`PARAMETER`';
		}

		$sql = sprintf(
			'INSERT INTO `nagios_host_check_command_parameter` (%s) VALUES (%s)',
