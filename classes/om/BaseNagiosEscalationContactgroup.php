...
abstract class BaseNagiosEscalationContactgroup extends BaseObject  implements Persistent
{
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = NagiosEscalationContactgroupQuery::create()
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(NagiosEscalationContactgroupPeer::ID)) {
			$modifiedColumns[':p' . $index++]  = '`ID`';
		}
		if ($this->isColumnModified(NagiosEscalationContactgroupPeer::ESCALATION)) {
			$modifiedColumns[':p' . $index++]  = '`ESCALATION`';
		}
		if ($this->isColumnModified(NagiosEscalationContactgroupPeer::CONTACTGROUP)) {
			$modifiedColumns[':p' . $index++]  = '`CONTACTGROUP`';
		}

		$sql = sprintf(
			'INSERT INTO `nagios_escalation_contactgroup` (%s) VALUES (%s)',
