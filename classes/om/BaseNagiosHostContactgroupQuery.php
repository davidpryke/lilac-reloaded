...
abstract class BaseNagiosHostContactgroupQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = NagiosHostContactgroupPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
	 ...
	 *
	 * @return    NagiosHostContactgroup A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con)
	{
		$sql = 'SELECT `ID`, `HOST`, `HOST_TEMPLATE`, `CONTACTGROUP` FROM `nagios_host_contactgroup` WHERE `ID` = :p0';
		try {
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$obj = new NagiosHostContactgroup();
			$obj->hydrate($row);
			NagiosHostContactgroupPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
