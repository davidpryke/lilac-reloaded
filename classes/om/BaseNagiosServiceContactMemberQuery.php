...
abstract class BaseNagiosServiceContactMemberQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = NagiosServiceContactMemberPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
	 ...
	 *
	 * @return    NagiosServiceContactMember A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con)
	{
		$sql = 'SELECT `ID`, `SERVICE`, `TEMPLATE`, `CONTACT` FROM `nagios_service_contact_member` WHERE `ID` = :p0';
		try {
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$obj = new NagiosServiceContactMember();
			$obj->hydrate($row);
			NagiosServiceContactMemberPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
