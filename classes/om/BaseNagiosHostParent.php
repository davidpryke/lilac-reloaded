...
abstract class BaseNagiosHostParent extends BaseObject  implements Persistent
{
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = NagiosHostParentQuery::create()
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(NagiosHostParentPeer::ID)) {
			$modifiedColumns[':p' . $index++]  = '`ID`';
		}
		if ($this->isColumnModified(NagiosHostParentPeer::CHILD_HOST)) {
			$modifiedColumns[':p' . $index++]  = '`CHILD_HOST`';
		}
		if ($this->isColumnModified(NagiosHostParentPeer::CHILD_HOST_TEMPLATE)) {
			$modifiedColumns[':p' . $index++]  = '`CHILD_HOST_TEMPLATE`';
		}
		if ($this->isColumnModified(NagiosHostParentPeer::PARENT_HOST)) {
			$modifiedColumns[':p' . $index++]  = '`PARENT_HOST`';
		}

		$sql = sprintf(
			'INSERT INTO `nagios_host_parent` (%s) VALUES (%s)',
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					case '`ID`':
						...
						break;
					case '`CHILD_HOST`':
						$stmt->bindValue($identifier, $this->child_host, PDO::PARAM_INT);
						break;
					case '`CHILD_HOST_TEMPLATE`':
						$stmt->bindValue($identifier, $this->child_host_template, PDO::PARAM_INT);
						break;
					case '`PARENT_HOST`':
						$stmt->bindValue($identifier, $this->parent_host, PDO::PARAM_INT);
						break;
				}
			}
			$stmt->execute();
