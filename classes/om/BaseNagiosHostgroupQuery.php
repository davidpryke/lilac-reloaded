...
abstract class BaseNagiosHostgroupQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = NagiosHostgroupPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
	 ...
	 *
	 * @return    NagiosHostgroup A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con)
	{
		$sql = 'SELECT `ID`, `NAME`, `ALIAS`, `NOTES`, `NOTES_URL`, `ACTION_URL` FROM `nagios_hostgroup` WHERE `ID` = :p0';
		try {
