...
abstract class BaseNagiosServiceGroupQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = NagiosServiceGroupPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
	 ...
	 *
	 * @return    NagiosServiceGroup A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con)
	{
		$sql = 'SELECT `ID`, `NAME`, `ALIAS`, `NOTES`, `NOTES_URL`, `ACTION_URL` FROM `nagios_service_group` WHERE `ID` = :p0';
		try {
