...
abstract class BaseNagiosServiceTemplateInheritance extends BaseObject  implements Persistent
{
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = NagiosServiceTemplateInheritanceQuery::create()
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(NagiosServiceTemplateInheritancePeer::ID)) {
			$modifiedColumns[':p' . $index++]  = '`ID`';
		}
		if ($this->isColumnModified(NagiosServiceTemplateInheritancePeer::SOURCE_SERVICE)) {
			$modifiedColumns[':p' . $index++]  = '`SOURCE_SERVICE`';
		}
		if ($this->isColumnModified(NagiosServiceTemplateInheritancePeer::SOURCE_TEMPLATE)) {
			$modifiedColumns[':p' . $index++]  = '`SOURCE_TEMPLATE`';
		}
		if ($this->isColumnModified(NagiosServiceTemplateInheritancePeer::TARGET_TEMPLATE)) {
			$modifiedColumns[':p' . $index++]  = '`TARGET_TEMPLATE`';
		}
		if ($this->isColumnModified(NagiosServiceTemplateInheritancePeer::ORDER)) {
			$modifiedColumns[':p' . $index++]  = '`ORDER`';
		}

		$sql = sprintf(
			'INSERT INTO `nagios_service_template_inheritance` (%s) VALUES (%s)',
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					case '`ID`':
						...
						break;
					case '`SOURCE_SERVICE`':
						$stmt->bindValue($identifier, $this->source_service, PDO::PARAM_INT);
						break;
					case '`SOURCE_TEMPLATE`':
						$stmt->bindValue($identifier, $this->source_template, PDO::PARAM_INT);
						break;
					case '`TARGET_TEMPLATE`':
						$stmt->bindValue($identifier, $this->target_template, PDO::PARAM_INT);
						break;
					case '`ORDER`':
