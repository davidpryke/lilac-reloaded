...
abstract class BaseNagiosServiceContactGroupMember extends BaseObject  implements Persistent
{
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = NagiosServiceContactGroupMemberQuery::create()
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(NagiosServiceContactGroupMemberPeer::ID)) {
			$modifiedColumns[':p' . $index++]  = '`ID`';
		}
		if ($this->isColumnModified(NagiosServiceContactGroupMemberPeer::SERVICE)) {
			$modifiedColumns[':p' . $index++]  = '`SERVICE`';
		}
		if ($this->isColumnModified(NagiosServiceContactGroupMemberPeer::TEMPLATE)) {
			$modifiedColumns[':p' . $index++]  = '`TEMPLATE`';
		}
		if ($this->isColumnModified(NagiosServiceContactGroupMemberPeer::CONTACT_GROUP)) {
			$modifiedColumns[':p' . $index++]  = '`CONTACT_GROUP`';
		}

		$sql = sprintf(
			'INSERT INTO `nagios_service_contact_group_member` (%s) VALUES (%s)',
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					...
					case '`TEMPLATE`':
						...
						break;
					case '`CONTACT_GROUP`':
						$stmt->bindValue($identifier, $this->contact_group, PDO::PARAM_INT);
						break;
				}
			}
			$stmt->execute();
