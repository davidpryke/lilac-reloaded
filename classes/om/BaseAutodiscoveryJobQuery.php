...
abstract class BaseAutodiscoveryJobQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = AutodiscoveryJobPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
	 ...
	 *
	 * @return    AutodiscoveryJob A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con)
	{
		$sql = 'SELECT `ID`, `NAME`, `DESCRIPTION`, `CONFIG`, `START_TIME`, `END_TIME`, `STATUS`, `STATUS_CODE`, `STATUS_CHANGE_TIME`, `STATS`, `CMD` FROM `autodiscovery_job` WHERE `ID` = :p0';
		try {
