...
abstract class BaseNagiosHostgroupMembershipQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = NagiosHostgroupMembershipPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
	 ...
	 *
	 * @return    NagiosHostgroupMembership A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con)
	{
		$sql = 'SELECT `ID`, `HOST`, `HOST_TEMPLATE`, `HOSTGROUP` FROM `nagios_hostgroup_membership` WHERE `ID` = :p0';
		try {
