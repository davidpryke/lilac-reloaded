...
abstract class BaseNagiosTimeperiodExclude extends BaseObject  implements Persistent
{
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = NagiosTimeperiodExcludeQuery::create()
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(NagiosTimeperiodExcludePeer::ID)) {
			$modifiedColumns[':p' . $index++]  = '`ID`';
		}
		if ($this->isColumnModified(NagiosTimeperiodExcludePeer::TIMEPERIOD_ID)) {
			$modifiedColumns[':p' . $index++]  = '`TIMEPERIOD_ID`';
		}
		if ($this->isColumnModified(NagiosTimeperiodExcludePeer::EXCLUDED_TIMEPERIOD)) {
			$modifiedColumns[':p' . $index++]  = '`EXCLUDED_TIMEPERIOD`';
		}

		$sql = sprintf(
			'INSERT INTO `nagios_timeperiod_exclude` (%s) VALUES (%s)',
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					case '`ID`':
						...
						break;
					case '`TIMEPERIOD_ID`':
						$stmt->bindValue($identifier, $this->timeperiod_id, PDO::PARAM_INT);
						break;
					case '`EXCLUDED_TIMEPERIOD`':
						$stmt->bindValue($identifier, $this->excluded_timeperiod, PDO::PARAM_INT);
						break;
				}
			}
			$stmt->execute();
