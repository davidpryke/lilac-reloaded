...
abstract class BaseNagiosHostContactMemberQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = NagiosHostContactMemberPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
	 ...
	 *
	 * @return    NagiosHostContactMember A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con)
	{
		$sql = 'SELECT `ID`, `HOST`, `TEMPLATE`, `CONTACT` FROM `nagios_host_contact_member` WHERE `ID` = :p0';
		try {
