...
abstract class BaseLilacConfigurationQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = LilacConfigurationPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
	 ...
	 *
	 * @return    LilacConfiguration A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con)
	{
		$sql = 'SELECT `ID`, `VERSION` FROM `lilac_configuration` WHERE `ID` = :p0';
		try {
