...
abstract class BaseNagiosContactGroupMemberQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = NagiosContactGroupMemberPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
	 ...
	 *
	 * @return    NagiosContactGroupMember A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con)
	{
		$sql = 'SELECT `ID`, `CONTACT`, `CONTACTGROUP` FROM `nagios_contact_group_member` WHERE `ID` = :p0';
		try {
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$obj = new NagiosContactGroupMember();
			$obj->hydrate($row);
			NagiosContactGroupMemberPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
