...
abstract class BaseImportJobQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = ImportJobPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
	 ...
	 *
	 * @return    ImportJob A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con)
	{
		$sql = 'SELECT `ID`, `NAME`, `DESCRIPTION`, `CONFIG`, `START_TIME`, `END_TIME`, `STATUS`, `STATUS_CODE`, `STATUS_CHANGE_TIME`, `STATS`, `CMD` FROM `import_job` WHERE `ID` = :p0';
		try {
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$obj = new ImportJob();
			$obj->hydrate($row);
			ImportJobPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
