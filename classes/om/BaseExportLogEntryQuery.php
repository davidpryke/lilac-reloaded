...
abstract class BaseExportLogEntryQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = ExportLogEntryPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
	 ...
	 *
	 * @return    ExportLogEntry A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con)
	{
		$sql = 'SELECT `ID`, `JOB`, `TIME`, `TEXT`, `TYPE` FROM `export_log_entry` WHERE `ID` = :p0';
		try {
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$obj = new ExportLogEntry();
			$obj->hydrate($row);
			ExportLogEntryPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
