...
abstract class BaseExportLogEntry extends BaseObject  implements Persistent
{
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = ExportLogEntryQuery::create()
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(ExportLogEntryPeer::ID)) {
			$modifiedColumns[':p' . $index++]  = '`ID`';
		}
		if ($this->isColumnModified(ExportLogEntryPeer::JOB)) {
			$modifiedColumns[':p' . $index++]  = '`JOB`';
		}
		if ($this->isColumnModified(ExportLogEntryPeer::TIME)) {
			$modifiedColumns[':p' . $index++]  = '`TIME`';
		}
		if ($this->isColumnModified(ExportLogEntryPeer::TEXT)) {
			$modifiedColumns[':p' . $index++]  = '`TEXT`';
		}
		if ($this->isColumnModified(ExportLogEntryPeer::TYPE)) {
			$modifiedColumns[':p' . $index++]  = '`TYPE`';
		}

		$sql = sprintf(
			'INSERT INTO `export_log_entry` (%s) VALUES (%s)',
