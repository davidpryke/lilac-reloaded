...
abstract class BaseNagiosHostTemplateInheritanceQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = NagiosHostTemplateInheritancePeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
	 ...
	 *
	 * @return    NagiosHostTemplateInheritance A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con)
	{
		$sql = 'SELECT `ID`, `SOURCE_HOST`, `SOURCE_TEMPLATE`, `TARGET_TEMPLATE`, `ORDER` FROM `nagios_host_template_inheritance` WHERE `ID` = :p0';
		try {
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$obj = new NagiosHostTemplateInheritance();
			$obj->hydrate($row);
			NagiosHostTemplateInheritancePeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
