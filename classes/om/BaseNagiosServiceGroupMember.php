...
abstract class BaseNagiosServiceGroupMember extends BaseObject  implements Persistent
{
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = NagiosServiceGroupMemberQuery::create()
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(NagiosServiceGroupMemberPeer::ID)) {
			$modifiedColumns[':p' . $index++]  = '`ID`';
		}
		if ($this->isColumnModified(NagiosServiceGroupMemberPeer::SERVICE)) {
			$modifiedColumns[':p' . $index++]  = '`SERVICE`';
		}
		if ($this->isColumnModified(NagiosServiceGroupMemberPeer::TEMPLATE)) {
			$modifiedColumns[':p' . $index++]  = '`TEMPLATE`';
		}
		if ($this->isColumnModified(NagiosServiceGroupMemberPeer::SERVICE_GROUP)) {
			$modifiedColumns[':p' . $index++]  = '`SERVICE_GROUP`';
		}

		$sql = sprintf(
			'INSERT INTO `nagios_service_group_member` (%s) VALUES (%s)',
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					...
					case '`TEMPLATE`':
						...
						break;
					case '`SERVICE_GROUP`':
						$stmt->bindValue($identifier, $this->service_group, PDO::PARAM_INT);
						break;
				}
			}
			$stmt->execute();
