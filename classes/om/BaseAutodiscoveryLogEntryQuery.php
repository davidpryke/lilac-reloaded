...
abstract class BaseAutodiscoveryLogEntryQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = AutodiscoveryLogEntryPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
	 ...
	 *
	 * @return    AutodiscoveryLogEntry A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con)
	{
		$sql = 'SELECT `ID`, `JOB`, `TIME`, `TEXT`, `TYPE` FROM `autodiscovery_log_entry` WHERE `ID` = :p0';
		try {
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$obj = new AutodiscoveryLogEntry();
			$obj->hydrate($row);
			AutodiscoveryLogEntryPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
