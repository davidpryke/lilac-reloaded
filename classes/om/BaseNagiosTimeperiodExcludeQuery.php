...
abstract class BaseNagiosTimeperiodExcludeQuery extends ModelCriteria
{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = NagiosTimeperiodExcludePeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
	 ...
	 *
	 * @return    NagiosTimeperiodExclude A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con)
	{
		$sql = 'SELECT `ID`, `TIMEPERIOD_ID`, `EXCLUDED_TIMEPERIOD` FROM `nagios_timeperiod_exclude` WHERE `ID` = :p0';
		try {
